...
const TableTimeCell = ({time, isTimeSecondCell, isBeingHovered, isOnStartingTimeSelection}) => {
  ...
  const timeMidClases = clsx({"bg-green-400 text-white font-semibold px-2 rounded-xl" : isBeingHovered && ((isOnStartingTimeSelection && isTimeSecondCell) || (!isOnStartingTimeSelection && !isTimeSecondCell))}, "text-sm transition-all duration-300")
